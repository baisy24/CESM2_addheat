...
!*********************** ADD HEAT EDITS **************************
! CL ADD IN ROUTINES USED                                    
! Adapted by HB to CAM6                                                              
!  use phys_grid,        only: get_ncols_p, get_lon_all_p, get_lat_all_p
  use pmgrid,           only: plev                           
!  use time_manager,     only: get_curr_date !CL/FA        
!  use cam_abortutils,   only: endrun !CAM6 --HB
!  use physconst,        only: cpair  !FA/CL 
! END CL ADD IN ROUTINES USED
!************************** END EDITS ****************************
...
integer, parameter :: iik = selected_real_kind(13,40) !CL/FA
   
integer, parameter :: n_ph = 1 !CL/FA
   
integer, dimension(n_ph), save :: month_to_start_heating,day_to_start_heating, month_to_stop_heating, day_to_stop_heating
   
real, dimension(n_ph),save ::hour_to_start_heating,hour_to_stop_heating !CL/FA
   
logical, dimension(n_ph), save :: do_heat !CL/FA
   
integer, save :: iilon0(n_ph) !iilon is the index of the longitude for Richter Box center CL/FA BAI
   
real(r8), save :: xxvweight(plev,n_ph) ! vertical weights of heating !CL/FA 
  
real(r8), save :: xxlat0, xxhcoef(n_ph), xxdistr !xxlat0 is the latitude where we are inserting the heating CL/FA
   
integer, save :: ispinup =0, nn !CL/FA
!************************* END EDITS ****************************
...
!*********************** ADD HEAT EDITS ************************
    use time_manager,       only: get_curr_date !CL/FA        
    use cam_history,        only: addfld, add_default !HB
    use ppgrid,             only: pcols, pver !CL/FA 
!************************** END EDITS **************************
...
!************************* ADD HEAT EDITS ***********************
    integer :: year, month, day, tod, iheattrue !CL/FA
    real :: hour

    real(r8), parameter :: xxwidth = 550.0 !CL/FA e-folding distance for heating
    real(r8) :: xxpi, xxtpid, xxarg
    real(r8) :: xxd, yarg, xxd_sq, xxdellon
    real(r8) :: rlon(pcols)  ! array of longitudes (radians)
    real(r8) :: rlat(pcols)  ! array of latitudes  (radians)

!    call addfld('XXH'  , 'K/day' , pver, 'A', 'Additional applied heating', phys_decomp) !CL/HB
!    call addfld('XXH     ', (/ 'lev' /), 'A', 'K/day', 'Additional applied heating')!, phys_decomp) !addfld format is diff in CAM6 --HB
!    call add_default('XXH     ', 1, ' ')  !CL
!*************************** END EDITS **************************
...
!*********************** ADD HEAT EDITS ***********************
!CL/FA : DEFINE PARAMETERS 

!*** Determine the longitude point for insertion of heating with Gaussian (x-y)
!spatial structure

!Set the location of heating here

iilon0(1)=125 ! Rbox, Index corresponds to 50W for f19_f19 resolution (1.9 deg lat*2.5 deg lon)  HB
!iilon0(1)=20 ! Index corresponds to 70 degrees E for f19_f19 resolution (1.9
!deg lat *2.5 deg lon) or "Phase 3"
!iilon0(1)=40 ! Index corresponds to 70 degrees E for f19_f19 resolution (1.9
!deg lat *2.5 deg lon) or "Phase 3"
!iilon0(1)=47 ! Index corresponds to 70 degrees E for f19_f19 resolution (1.9
!deg lat *2.5 deg lon) or "Phase 3"

!iilon0(1)=62 ! Index corresponds to 155 degrees E for f19_f19 resolution (1.9
!deg lat *2.5 deg lon) or "Phase 3"

!Latitude point in radians for insertion of heaitng

xxlat0=0.0698_r8 !4N in radians
!xxhcoef(1) =2e-14+4.0d0/8.64d+04 ! Convert from 4 deg/day to deg/sec - "1X" 
!xxhcoef(1) =-3.0d0/8.64d+04 ! Convert from 1 deg/day to deg/sec - "2X" 
!xxhcoef(1) =4.5d0/8.64d+04 ! Convert from 1 deg/day to deg/sec - "3X" 


!xxhcoef(1) =0.0 

!*** xxwidth gives the e-folding distance in km of the isotropic Gaussian
!horizontal structure computed on the sphere

!*** the present e-folding distance is about 13.5 degrees at the equator.

xxdistr=6371.0d0/xxwidth !6371 is the radius of the earth in Kms
xxpi=acos(-1.0d0)
xxtpid=2.0d0*xxpi/real(2*plev,iik)

!*** Define vertical weights (as in equation 2 in the Straus text):

xxvweight(:,:)=0.0

!mid heavy profile to simulate convective heating  BAI
!xxvweight(28,:) = 0.1798
!xxvweight(27,:) = 0.2518
!xxvweight(26,:) = 0.3305
!xxvweight(25,:) = 0.4148
...
7378 format('k =',i5,' xxvweight(k) = ',1pd13.5)


!month_to_start_heating(1) = 3; month_to_stop_heating(1) = 4
!day_to_start_heating(1) = 1 ; day_to_stop_heating(1) = 1
!hour_to_start_heating(1) =0 ; hour_to_stop_heating(1)=0
...
!************************ ADD HEAT EDITS ***********************
    use time_manager,    only: get_curr_date !CL/FA
    use pmgrid,          only: plev      !HB
    use phys_grid,       only: get_ncols_p, get_lon_all_p, get_lat_all_p !HB
    use cam_history,     only: outfld    !HB
!**************************** END EDITS ************************
...
!********************** ADD HEAT EDITS ***********************
   integer :: lats(pcols), lons(pcols), i, m, k, mp1
   integer :: year, month, day, tod, iheattrue, iicol !CL/FA
   integer, parameter :: nphase = 1
   real :: hour !FA   

   real(r8) :: xxd, xxd2, yarg, xxd_sq,yarg2,xxd_sq2, xxdellon,xxdellon2
   real(r8) :: xxlat,wt(2)
   real(r8) :: rlon(pcols) ! array of longitudes (radians) !CL/FA
   real(r8) :: rlat(pcols) ! array of latitudes  (radians) !CL/FA
   real(r8) :: pmid(pcols,plev), pint(pcols,plev +1), xxh(pcols, plev) ! array of latitudes (radians) !CL/FA
!   real(r8) :: xxh(pcols, plev) ! array of latitudes (radians) !HB

   call get_curr_date(year, month, day, tod)
   hour=tod*1.0/3600 ! convert time of day in seconds to hour of day
!************************* END EDITS *************************
...
!if(month_to_stop_heating(1).eq.month_to_start_heating(1)) then
!
!      if(month_to_start_heating(1) == month) then
!
!        if((day>=day_to_start_heating(1)).and.(day<=day_to_stop_heating(1)))
!        then
!
 !            !if((hour>=hour_to_start_heating(1)).and.(hour<=hour_to_stop_heating(1)))
 !            then
!
 !            if((day ==
 !            day_to_start_heating(1).and.hour>=hour_to_start_heating(1)).or.(day
 !            > day_to_start_heating(1) .and.(day<day_to_stop_heating(1)))) then
!
 !                 do_heat(1) = .true.
!
 !            else if (day == day_to_stop_heating(1) .and.
 !            hour<=hour_to_stop_heating(1)) then
!
 !                 do_heat(1) = .true.
  !            else
   !               do_heat(1) = .false.
...
!
 !    else
  !      do_heat(1) = .false.
   !  endif
!
!else if(month_to_start_heating(1) == month) then
!
 !         if((day >= day_to_start_heating(1))) then
 !
  !          if((hour >= hour_to_start_heating(1))) then
   !              do_heat(1) = .true.
    !          else
     !            do_heat(1) = .false.
      !        endif
       !   else
        !     do_heat(1) = .false.
         ! endif
!
!else if(month_to_stop_heating(1) == month) then
!
 !      if((day <= day_to_stop_heating(1))) then
!
 !        if((hour <= hour_to_stop_heating(1))) then
!
 !            do_heat(1) =.true.
  !           else
   !          do_heat(1) = .false.
...
!
 !      end if
!
!else if((month_to_start_heating(1) < month) .and. (month_to_stop_heating(1) >
!month)) then
 !     do_heat(1) = .true.
!
!end if
...
! heating rates correspond to xxvweight=1 for 12-14:00(local time) every day
!--HB
xxhcoef(1) = 0.0
if ((tod >= 57600) .and. (tod <= 64800)) then
 if (month == 1) then
! if ((month == 3) .or. (month == 4) .or. (month == 5)) then
  do_heat(1) = .true.
  xxhcoef(1) =2e-14+18.0d0/8.64d+04
! if ((month == 3) .or. (month == 4)) then
!  do_heat(1) = .true.
!  xxhcoef(1) =2e-14+18.0d0/8.64d+04 ! Convert from 18 deg/day to deg/sec
! else if (month == 5) then
!  do_heat(1) = .true.
!  xxhcoef(1) =2e-14+15.0d0/8.64d+04 ! Convert from 12 deg/day to deg/sec
 else
  do_heat(1) = .false.
  xxhcoef(1) = 0.0
 endif
...
!************************ ADD HEAT EDITS **********************
!Add in call to get latitude and longitude incides

       call get_lon_all_p(c, ncol, lons)
       call get_lat_all_p(c, ncol, lats)

     do iicol=1,ncol
       rlat(iicol) = phys_state(c)%lat(iicol)   !this gives the lat in radians
       rlon(iicol) = phys_state(c)%lon(iicol)   !this gives the lon in radians
     end do

!***** START HEATING IMPLEMENTATION - CL/FA *****

!modified to add heating to the coast of east amazon - HB 
phys_state(c)%xxh = 0._r8
     xxh  = 0._r8
     m=1
     do while(.true.)
        if(do_heat(m)) then
           ...
           do k=1,plev
            ...
            !  xxlat = rlat(i)
              if (((lats(i)==97) .and. (lats(i)<=103)) .and. ((lons(i)>=242) .and. (lons(i)<=246))) then
              phys_state(c)%xxh(i,k) = xxhcoef(m)*xxvweight(k,m)
!!               else if ((lats(i)==101) .and. ((lons(i)>=242) .and. (lons(i)<=244))) then
!!              phys_state(c)%xxh(i,k) = xxhcoef(m)*xxvweight(k,m)
!!               else if ((lats(i)==100) .and. ((lons(i)>=243) .and. (lons(i)<=245))) then
!!              phys_state(c)%xxh(i,k) = xxhcoef(m)*xxvweight(k,m)
!!               else if ((lats(i)==99) .and. ((lons(i)>=244) .and. (lons(i)<=245))) then
!!              phys_state(c)%xxh(i,k) = xxhcoef(m)*xxvweight(k,m)
!!               else if ((lats(i)==98) .and. ((lons(i)>=245) .and. (lons(i)<=247))) then
!!              phys_state(c)%xxh(i,k) = xxhcoef(m)*xxvweight(k,m)
           !    else if ((lats(i)==47) .and. (lons(i)==127)) then
           !   phys_state(c)%xxh(i,k) = xxhcoef(m)*xxvweight(k,m)
              else
                phys_state(c)%xxh(i,k) = 0.0
              endif

              xxh(i,k) = phys_state(c)%xxh(i,k)

!*** The heating due to xxh will be added to the temperature tendency in the
!routine tphysac
             enddo
            enddo
           exit
 ...
 7776 format(' i = ',i4,' k = ',i5,' lats(i) = ',i5,' lons(i) = ',i5,' rlat(i) =',1pd12.3,' rlon(i) = ',1pd12.3,' xxh = ',e15.8)


!  call outfld('XXH     ', xxh, pcols , c)

!***** Heating is maximum at k=20 *****

     do iicol=1,ncol
       rlat(iicol) = phys_state(c)%lat(iicol)   !lats gives the number index of a particular lat, while rlat gives the lat in radians
       rlon(iicol) = phys_state(c)%lon(iicol)   !lons gives the number index of a particular lon, while rlon gives the lon in radians

       pmid(iicol,:) = phys_state(c)%pmid(iicol,:)
       pint(iicol,:) = phys_state(c)%pint(iicol,:)
...
!************************ ADD HEAT EDITS ************************
    type(physics_ptend)     :: ptend_added         ! create another "ptend" copy to not mistakenly impact the other parameterizations --HB
!************************** END EDITS ***************************
...
!*********************** ADD HEAT EDITS ************************
    real(r8) xxh(pcols,pver)  !heating
!    real(r8) pmid(pcols,plev) !HB
!    real(r8) pint(pcols,plev +1) !HB
!************************** END EDITS **************************
...
!********************** ADD HEAT EDITS ***********************
! ADD HEATING TO TENDENCY
  ptend_added%top_level = 1
  ptend_added%bot_level = pver

  ptend_added%psetcols = state%psetcols
  ptend_added%name = "CLs heating"
  ptend_added%lq(:) = .false.
  ptend_added%ls    = .true. !won't run if set to true
  ptend_added%lu    = .false.
  ptend_added%lv    = .false.

  ptend_added%s     = xxh*cpair

  call physics_update(state, ptend_added, ztodt, tend)
!************************* END EDITS *************************
